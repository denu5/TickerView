...
import { Page, Text, Card, Badge, Spacer, ButtonGroup, Button } from '@geist-ui/core';
...
const Home: NextPage<ITickers> = ({ results }) => {
  return (
    <Page>
      ...
      {results.map(({ ticker, name, market, composite_figi, type }) => (
        <NextLink key={ticker} href={`/ticker/${ticker}`} passHref>
          <a>
            <Card shadow mb={2} width="100%">
              <Text h4 my={0}>{ticker}</Text>
              <Text my={0}>{name}</Text>
              <Card.Footer>
                {[market, type, composite_figi].map(feature => (feature && <Badge type="secondary" mr={1}>{feature.toUpperCase()}</Badge>))}
              </Card.Footer>
            </Card>
          </a>
        </NextLink>
